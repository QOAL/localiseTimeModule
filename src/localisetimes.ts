...
function localiseInput(input: string): string {
	let newText: string
	let mode = "t"
...
function spotTime(str: string, mode: string = "t"): Array<localisedTimeInfo> {
	...
	matches.forEach(match => {
		...
		let tmpDate = new Date(
			...
			tmpExplode[0],
			tmpExplode[1],
			match[_G.seconds] ? Number(match[_G.seconds].substring(1)) : 0
		)
		...
		if (match[_G.startHour] && validMidnight) {
			...
			let tmpDate = new Date(
				...
				tmpExplode[0],
				tmpExplode[1],
				match[_G.startSeconds] ? Number(match[_G.startSeconds].substring(1)) : 0
			)
...
function validateTime(match: RegExpMatchArray, str: string, upperTZ: string, usingManualTZ: boolean = false): boolean {
	...
	if (
		requiresMeridiemOrMinsWithLowercase.includes(match[_G.tzAbr]) &&
